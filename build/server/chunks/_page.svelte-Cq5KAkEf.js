...
function MemberCard($$payload, $$props) {
  ...
  $$payload.out += `<div class="border-b flex justify-between items-center py-2"><div><p class="text-sm text-primary">สมาชิกลำดับที่ ${escape_html(index)}</p> <p class="text-lg">${escape_html(member.name)}</p></div> <img${attr("src", member.profileImage)}${attr("alt", member.name)} width="50" height="50" class="rounded-full"></div>`;
  pop();
}
...
//# sourceMappingURL=_page.svelte-Cq5KAkEf.js.map
