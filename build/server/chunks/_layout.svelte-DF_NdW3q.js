import { S as escape_html, I as attr, J as store_get, K as unsubscribe_stores, n as pop, L as rest_props, E as fallback, M as spread_props, G as bind_props, p as push, F as slot, O as spread_attributes, P as sanitize_props } from './index-CohHun4k.js';
import { c as cn } from './utils-GiUy8APZ.js';
import { m as makeElement } from './create-P9acJvsF.js';
...
function _layout($$payload, $$props) {
  ...
  $$payload.out += `<div class="p-8 flex flex-col gap-4"><a href="/party" class="underline">ย้อนกลับ</a> <div class="flex justify-between w-full bg-gradient-to-t from-[#27374D] to-[#9DB2BF] p-4 rounded-lg text-white shadow"><div class="flex flex-col justify-between"><div><h1 class="text-2xl font-bold">พรรค${escape_html(party.name)}</h1> <p class="text-lg">${escape_html(candidate.partyMember?.name)}</p> <span>${escape_html(party.votes)} คะแนน</span></div> <p class="text-2xl font-bold">ลำดับที่ ${escape_html(ranking)}</p></div> <img${attr("src", party.image)}${attr("alt", party.name)} width="150" height="150" class="rounded-full"></div> <ul class="flex gap-2"><li${attr("class", cn({
...
//# sourceMappingURL=_layout.svelte-DF_NdW3q.js.map
