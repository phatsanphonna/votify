import { N as ensure_array_like, M as spread_props, n as pop, I as attr, S as escape_html, p as push } from './index-CohHun4k.js';

function PartyCard($$payload, $$props) {
  let { image, name, votes, href } = $$props;
  $$payload.out += `<a${attr("href", `party/${href}`)} class="shadow bg-gradient-to-t from-[#27374D] to-[#9DB2BF] rounded-2xl p-4 flex flex-col gap-2 text-center justify-center items-center h-64"><img${attr("src", image)} width="100" height="100"${attr("alt", `${name} party photo`)} class="rounded-full"> <p class="text-2xl font-bold text-white">${escape_html(name)}</p> <p class="text-white">${escape_html(votes)} คะแนน</p></a>`;
}
...
//# sourceMappingURL=_page.svelte-fiXrPV-j.js.map
